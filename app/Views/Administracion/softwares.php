                ...
                <table id="datatable" class="table table-bordered table-bordered dt-responsive nowrap text-center">
                  <thead>
                    <tr class="text-center">
                      <th> <?= ucfirst(lang('App.software')) ?> </th>
              ...
              <div class="card-box">
                ...
                <form id="form-1" class="form-horizontal" role="form" autocomplete="off">
                  <div class="form-group row">
                    <label for="software" class="col-sm-3 col-form-label"> <?= lang('App.software') ?>: </label>
                    <div class="col-sm-9">
                      <input type="text" id="software" name="software" class="form-control" placeholder="<?= lang('App.software') ?>">
                    </div>
                  </div>
                  <div class="form-group row">
  ...
  <script>
    ...
    $(document).ready(function(){
      ...
      function creaTabla(){
        table =  $('#datatable').DataTable({
          ...
          "ajax" : {
            "url" : "<?= base_url('admin/catalogs/softwares/table') ?>",
            "type" : "GET"
          },
          "columns" : [
            {'data' : 'software'},
    ...
    $('#form-1').submit(function(e){
      ...
      if ($("#form-1").valid() === true) {
        $.ajax({
          url : '<?= base_url('admin/catalogs/softwares') ?>',
          type : 'POST',
    ...
    $('#form-2').submit(function(e){
      ...
      e.preventDefault();
      $.ajax({
        url : '<?= base_url('admin/catalogs/softwares') ?>/'+id,
        method : "PUT",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
    ...
    function editar(id) {
      $.ajax('<?= base_url('admin/catalogs/softwares') ?>/'+ id, {
        method : "GET",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        success : function(res) {
          let data = res;
          $('#id').val(data.response.softwareId);
          $('#form-2  #software').val(data.response.software);
          $("#form-2  option[value='"+data.response.softwareTypeSoftwareId+"']").attr("selected", true);
        }
      })
