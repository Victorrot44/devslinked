                ...
                <table id="datatable" class="table table-bordered table-bordered dt-responsive nowrap text-center">
                  <thead>
                    <tr class="text-center">
                      <th> <?= ucfirst(lang('App.database')) ?> </th>
              ...
              <div class="card-box">
                ...
                <form id="form-1" class="form-horizontal" role="form" autocomplete="off">
                  <div class="form-group row">
                    <label for="database" class="col-sm-3 col-form-label"> <?= lang('App.database') ?>: </label>
                    <div class="col-sm-9">
                      <input type="text" id="database" name="database" class="form-control" placeholder="<?= lang('App.database') ?>">
                    </div>
                  </div>
  ...
  <script>
    ...
    $(document).ready(function(){
      ...
      $('#form-1').validate({
        lang : 'es',
        rules : {
          database : { required: true, maxlength: 40 }
        },
      ...
      function creaTabla(){
        table =  $('#datatable').DataTable({
          ...
          "ajax" : {
            "url" : "<?= base_url('admin/catalogs/databases/table') ?>",
            "type" : "GET"
          },
          "columns" : [
            {'data' : 'database'},
    ...
    $('#form-1').submit(function(e){
      e.preventDefault();
      $.ajax({
        url : '<?= base_url('admin/catalogs/databases') ?>',
        type : 'POST',
    ...
    $('#form-2').submit(function(e){
      ...
      e.preventDefault();
      $.ajax({
        url : '<?= base_url('admin/catalogs/databases') ?>/'+id,
        method : "PUT",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        contentType: 'application/json',
        dataType: 'json',
        data: JSON.stringify({
          "database"   : ""+$('#form-2  #database').val()+""
        }),
        success : function(res) {
          table.ajax.reload();
          resetCss();
    ...
    function editar(id) {
      $.ajax('<?= base_url('admin/catalogs/databases') ?>/'+ id, {
        method : "GET",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        success : function(res) {
          let data = res;
          $('#id').val(data.response.databaseManagerId);
          $('#form-2  #database').val(data.response.databaseManager);
        }
      })
