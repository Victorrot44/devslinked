              ...
              <div class="card-box">
                ...
                <form id="form-1" class="form-horizontal" role="form" autocomplete="off">
                  <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                      <input type="text" id="type" name="type" class="form-control" placeholder="<?= lang('App.type_software') ?>">
                    </div>
                  </div>
  ...
  <script>
    ...
    $(document).ready(function(){
      ...
      $('#form-1').validate({
        lang : 'es',
        rules : {
          languages : { required: true, maxlength: 40 }
        },
      ...
      function creaTabla(){
        table =  $('#datatable').DataTable({
          ...
          "ajax" : {
            "url" : "<?= base_url('admin/catalogs/type-software/table') ?>",
            "type" : "GET"
          },
          "columns" : [
            {'data' : 'system'},
    ...
    $('#form-1').submit(function(e){
      ...
      if ($("#form-1").valid() === true) {
        $.ajax({
          url : '<?= base_url('admin/catalogs/type-software') ?>',
          type : 'POST',
    ...
    $('#form-2').submit(function(e){
      ...
      e.preventDefault();
      $.ajax({
        url : '<?= base_url('admin/catalogs/type-software') ?>/'+id,
        method : "PUT",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
    ...
    function editar(id) {
      $.ajax('<?= base_url('admin/catalogs/type-software') ?>/'+ id, {
        method : "GET",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        success : function(res) {
          let data = res;
          $('#id').val(data.response.typeSoftwareId);
          $('#form-2  #type').val(data.response.typeSoftware);
        }
      })
