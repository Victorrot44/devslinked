                ...
                <table id="datatable" class="table table-bordered table-bordered dt-responsive nowrap text-center">
                  <thead>
                    <tr class="text-center">
                      <th> <?= ucfirst(lang('App.framework')) ?> </th>
                      <th> <?= ucfirst(lang('App.programming_language')) ?> </th>
              ...
              <div class="card-box">
                ...
                <form id="form-1" class="form-horizontal" role="form" autocomplete="off">
                  <div class="form-group row">
                    <label for="framework" class="col-sm-3 col-form-label"> <?= lang('App.framework') ?>: </label>
                    <div class="col-sm-9">
                      <input type="text" id="framework" name="framework" class="form-control" placeholder="<?= lang('App.framework') ?>">
                    </div>
                  </div>
                  <div class="form-group row">
                    <label for="language" class="col-sm-3 col-form-label"> <?= lang('App.programming_language') ?>: </label>
                    <div class="col-sm-9">
                      <select name="language" id="language" class="custom-select"></select>
                    </div>
                  </div>
...
<?= $this->section('js') ?>
  ...
  <script>
    ...
    $(document).ready(function(){
      ...
      $.getJSON("<?= base_url('admin/catalogs/programming-languages/all') ?>", function(data) {
        var opciones = $('#form-1 select , #form-2  select');
        $('#form-1 select , #form-2  select').empty();
        opciones.append($("<option />").val('').text('SELECCIONE ...'));
        // console.log(data);
        $.each(data.response, function () {
          opciones.append($("<option />").val(this.programmingLanguageId).text(this.programmingLanguage));
        });
      });
      ...
      $('#form-1').validate({
        lang : 'es',
        rules : {
          framework : { required: true, maxlength: 40 },
          language : "required"
        },
      ...
      function creaTabla(){
        table =  $('#datatable').DataTable({
          ...
          "ajax" : {
            "url" : "<?= base_url('admin/catalogs/frameworks/table') ?>",
            "type" : "GET"
          },
          "columns" : [
            {'data' : 'framework'},
    ...
    $('#form-1').submit(function(e){
      e.preventDefault();
      $.ajax({
        url : '<?= base_url('admin/catalogs/frameworks') ?>',
        type : 'POST',
    ...
    $('#form-2').submit(function(e){
      ...
      e.preventDefault();
      $.ajax({
        url : '<?= base_url('admin/catalogs/frameworks') ?>/'+id,
        method : "PUT",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        contentType: 'application/json',
        dataType: 'json',
        data: JSON.stringify({
          "framework"   : ""+$('#form-2  #framework').val()+"",
          "language"    : ""+$('#form-2  #language').val()+"",
        }),
        success : function(res) {
          table.ajax.reload();
          resetCss();
    ...
    function editar(id) {
      $.ajax('<?= base_url('admin/catalogs/frameworks') ?>/'+ id, {
        method : "GET",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        success : function(res) {
          let data = res;
          $('#id').val(data.response.frameworkId);
          $('#form-2  #framework').val(data.response.framework);
          $("#form-2 option[value='"+data.response.frameworkProgrammingLanguageId+"']").attr("selected", true);
        }
      })
