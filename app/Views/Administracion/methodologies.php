                ...
                <table id="datatable" class="table table-bordered table-bordered dt-responsive nowrap text-center">
                  <thead>
                    <tr class="text-center">
                      <th> <?= ucfirst(lang('App.methodology')) ?> </th>
              ...
              <div class="card-box">
                ...
                <form id="form-1" class="form-horizontal" role="form" autocomplete="off">
                  <div class="form-group row">
                    <label for="methodology" class="col-sm-3 col-form-label"> <?= lang('App.methodology') ?>: </label>
                    <div class="col-sm-9">
                      <input type="text" id="methodology" name="methodology" class="form-control" placeholder="<?= lang('App.methodology') ?>">
                    </div>
                  </div>
  ...
  <script>
    ...
    $(document).ready(function(){
      ...
      $('#form-1').validate({
        lang : 'es',
        rules : {
          methodology : { required: true, maxlength: 40 }
        },
      ...
      function creaTabla(){
        table =  $('#datatable').DataTable({
          ...
          "ajax" : {
            "url" : "<?= base_url('admin/catalogs/methodologies/table') ?>",
            "type" : "GET"
          },
          "columns" : [
            {'data' : 'system'},
    ...
    $('#form-1').submit(function(e){
      ...
      if ($("#form-1").valid() === true) {
        $.ajax({
          url : '<?= base_url('admin/catalogs/methodologies') ?>',
          type : 'POST',
    ...
    $('#form-2').submit(function(e){
      ...
      e.preventDefault();
      $.ajax({
        url : '<?= base_url('admin/catalogs/methodologies') ?>/'+id,
        method : "PUT",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        contentType: 'application/json',
        dataType: 'json',
        data: JSON.stringify({
          "methodology"   : ""+$('#form-2  #methodology').val()+""
        }),
        success : function(res) {
          table.ajax.reload();
          resetCss();
    ...
    function editar(id) {
      $.ajax('<?= base_url('admin/catalogs/methodologies') ?>/'+ id, {
        method : "GET",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        success : function(res) {
          let data = res;
          $('#id').val(data.response.methodologyId);
          $('#form-2  #methodology').val(data.response.methodology);
        }
      })
