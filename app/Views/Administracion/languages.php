                ...
                <table id="datatable" class="table table-bordered table-bordered dt-responsive nowrap text-center">
                  <thead>
                    <tr class="text-center">
                      <th> <?= ucfirst(lang('App.language')) ?> </th>
              ...
              <div class="card-box">
                ...
                <form id="form-1" class="form-horizontal" role="form" autocomplete="off">
                  <div class="form-group row">
                    <label for="language" class="col-sm-3 col-form-label"> <?= lang('App.language') ?>: </label>
                    <div class="col-sm-9">
                      <input type="text" id="language" name="language" class="form-control" placeholder="<?= lang('App.language') ?>">
                    </div>
                  </div>
  ...
  <script>
    ...
    $(document).ready(function(){
      ...
      function creaTabla(){
        table =  $('#datatable').DataTable({
          ...
          "ajax" : {
            "url" : "<?= base_url('admin/catalogs/languages/table') ?>",
            "type" : "GET"
          },
          "columns" : [
            {'data' : 'system'},
    ...
    $('#form-1').submit(function(e){
      ...
      if ($("#form-1").valid() === true) {
        $.ajax({
          url : '<?= base_url('admin/catalogs/languages') ?>',
          type : 'POST',
    ...
    $('#form-2').submit(function(e){
      ...
      e.preventDefault();
      $.ajax({
        url : '<?= base_url('admin/catalogs/languages') ?>/'+id,
        method : "PUT",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
    ...
    function editar(id) {
      $.ajax('<?= base_url('admin/catalogs/languages') ?>/'+ id, {
        method : "GET",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        success : function(res) {
          let data = res;
          $('#id').val(data.response.languageId);
          $('#form-2  #language').val(data.response.language);
        }
      })
