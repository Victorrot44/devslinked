                ...
                <table id="datatable" class="table table-bordered table-bordered dt-responsive nowrap text-center">
                  <thead>
                    <tr class="text-center">
                      <th> <?= ucfirst(lang('App.tool')) ?> </th>
                      <th> <?= ucfirst(lang('App.type_software')) ?> </th>
              ...
              <div class="card-box">
                ...
                <form id="form-1" class="form-horizontal" role="form" autocomplete="off">
                  <div class="form-group row">
                    <label for="software" class="col-sm-3 col-form-label"> <?= lang('App.cloud-tools') ?>: </label>
                    <div class="col-sm-9">
                      <input type="text" id="software" name="software" class="form-control" placeholder="<?= lang('App.cloud-tools') ?>">
                    </div>
                  </div>
                  <div class="form-group row">
                    <label for="type" class="col-sm-3 col-form-label"> <?= lang('App.type_software') ?>: </label>
                    <div class="col-sm-9">
...
<?= $this->section('js') ?>
  ...
  <script>
    ...
    $(document).ready(function(){
      ...
      $.getJSON("<?= base_url('admin/catalogs/type-software/all') ?>", function(data) {
        var opciones = $('#form-1  select , #form-2  select');
        $('#form-1  select , #form-2  select').empty();
        opciones.append($("<option />").val('').text('SELECCIONE ...'));
        // console.log(data);
        $.each(data.response, function () {
          opciones.append($("<option />").val(this.typeSoftwareId).text(this.typeSoftware));
        });
      });
      ...
      $('#form-1').validate({
        lang : 'es',
        rules : {
          software : { required: true, maxlength: 100 },
          type : "required"
        },
      ...
      function creaTabla(){
        table =  $('#datatable').DataTable({
          ...
          "ajax" : {
            "url" : "<?= base_url('admin/catalogs/cloud-tools/table') ?>",
            "type" : "GET"
          },
          "columns" : [
            {'data' : 'software'},
    ...
    $('#form-1').submit(function(e){
      ...
      if ($("#form-1").valid() === true) {
        $.ajax({
          url : '<?= base_url('admin/catalogs/cloud-tools') ?>',
          type : 'POST',
    ...
    $('#form-2').submit(function(e){
      ...
      e.preventDefault();
      $.ajax({
        url : '<?= base_url('admin/catalogs/cloud-tools') ?>/'+id,
        method : "PUT",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        contentType: 'application/json',
        dataType: 'json',
        data: JSON.stringify({
          "type"   : ""+$('#form-2  #type').val()+"",
          "software" : ""+$('#form-2  #software').val()+"",
        }),
        success : function(res) {
          table.ajax.reload();
          resetCss();
    ...
    function editar(id) {
      $.ajax('<?= base_url('admin/catalogs/cloud-tools') ?>/'+ id, {
        method : "GET",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        success : function(res) {
          let data = res;
          $('#id').val(data.response.cloudToolId);
          $('#form-2  #software').val(data.response.cloudTool);
          $("#form-2  option[value='"+data.response.cloudToolTypeSoftwareId+"']").attr("selected", true);
        }
      })
