              ...
              <div class="card-box">
                <form id="form-1" class="form-horizontal" role="form" autocomplete="off">
                  <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                      <input type="text" id="language" name="language" class="form-control" placeholder="<?= lang('App.programming_language') ?>">
                    </div>
                  </div>
                  <div class="form-group mb-0 justify-content-end row">
                    <div class="col-sm-9">
                      <button type="submit" class="btn btn-info waves-effect waves-light">
                        <i class="fa fa-save"></i> <?= ucfirst(lang('Add.save')) ?>
                      </button>
                    </div>
                  </div>
                </form>
  ...
  <script>
    ...
    $(document).ready(function(){
      ...
      $('#form-1').validate({
        lang : 'es',
        rules : {
          language : { required: true, maxlength: 40 }
        },
      ...
      function creaTabla(){
        table =  $('#datatable').DataTable({
          ...
          "ajax" : {
            "url" : "<?= base_url('admin/catalogs/programming-languages/table') ?>",
            "type" : "GET"
          },
          "columns" : [
            {'data' : 'language'},
    ...
    $('#form-1').submit(function(e){
      e.preventDefault();
      $.ajax({
        url : '<?= base_url('admin/catalogs/programming-languages') ?>',
        type : 'POST',
    ...
    $('#form-2').submit(function(e){
      ...
      e.preventDefault();
      $.ajax({
        url : '<?= base_url('admin/catalogs/programming-languages') ?>/'+id,
        method : "PUT",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
    ...
    function editar(id) {
      $.ajax('<?= base_url('admin/catalogs/programming-languages') ?>/'+ id, {
        method : "GET",
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        success : function(res) {
          let data = res;
          $('#id').val(data.response.programmingLanguageId);
          $('#form-2  #language').val(data.response.programmingLanguage);
        }
      })
