...
use App\Models\Catalogos\SoftwareModel; 
...
class SoftwareController extends BaseController{
  public function __construct() {
    ...
    if ($this->request->getMethod() == 'post'):
      ...
      $data = [
        'software' => trim(ucfirst($this->request->getPost('software'))),
        'softwareTypeSoftwareId' => trim(ucfirst($this->request->getPost('type'))),
      ];
      
      $this->model->newRecord($data);
      return $this->response->setStatusCode(201);
    else :
      $data['titulo'] = lang('App.softwares');
      return view('Administracion/softwares', $data);
    endif;
	}
  ...
  public function tabla(){
    ...
    foreach($data as $item):
      ...
      $accion .= '<button type=\"button\" class=\"btn btn-primary waves-effect\" title=\"'.lang('App.update').'\" onclick=\"editar('. $item->softwareId .')\" data-toggle=\"modal\" data-target=\"#modalEdit\">';
      $accion .= '<i class=\"fa fa-edit\"></i> </button>';
      $accion .= '<button type=\"button\" class=\"btn btn-danger waves-effect\" title=\"'.lang('App.delete').'\" onclick=\"eliminar('. $item->softwareId .')\">';
  ...
  public function actualizarRegistro(int $id) {
    ...
    $data = [
      'software' => trim($this->request->getJsonVar('software')),
      'softwareTypeSoftwareId' => trim($this->request->getJsonVar('type'))
    ];
