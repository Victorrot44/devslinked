...
use App\Models\Catalogos\OperatingSystemModel;
...
class OperatingSystemController extends BaseController{
  public function __construct() {
    ...
    if ($this->request->getMethod() == 'post'):
      if (!$this->validate(['system' => 'required|max_length[50]'])):
        $res = [
          'error'         => true,
          'code_error'    => 400,
      ...
      $data = ['operatingSystem' => trim(ucfirst($this->request->getPost('system')))];
      
      $this->model->newRecord($data);
      return $this->response->setStatusCode(201);
    else :
      $data['titulo'] = lang('App.operating_systems');
      return view('Administracion/operating-system', $data);
    endif;
	}
  ...
  public function tabla(){
    ...
    foreach($data as $item):
      ...
      $accion .= '<button type=\"button\" class=\"btn btn-primary waves-effect\" title=\"'.lang('App.update').'\" onclick=\"editar('. $item->operatingSystemId .')\" data-toggle=\"modal\" data-target=\"#modalEdit\">';
      $accion .= '<i class=\"fa fa-edit\"></i> </button>';
      $accion .= '<button type=\"button\" class=\"btn btn-danger waves-effect\" title=\"'.lang('App.delete').'\" onclick=\"eliminar('. $item->operatingSystemId .')\">';
      $accion .= '<i class=\"fas fa-trash-alt\"></i> </button> </div>';
      $tabla .= '{
        "system" : "'.$item->operatingSystem.'",
        "acciones"  : "'.$accion.'"
      },';
    endforeach;
  ...
  public function actualizarRegistro(int $id) {
    ...
    $data = ['operatingSystem' => trim($this->request->getJsonVar('system'))];
